...
///
/// Emitted by the OAuth body rewriters (response hook in `forward.rs`,
/// driven from both the TLS-intercept and reverse-proxy paths) every
/// time real upstream OAuth tokens are swapped for broker-issued
/// nonces. Capture is privileged — the in-process broker controls
/// real Anthropic credentials — so each event is recorded at `info!`
/// level *and* persisted to the audit ring buffer with a stable
/// `OAUTH_CAPTURE` reason prefix.
///
/// `fields_substituted` is the number of token fields rewritten in
/// this response (1 for access-only or refresh-only, 2 for the full
/// pair). Token and nonce values are deliberately NOT logged —
/// neither full nor partial — so the audit trail captures the *act*
/// of capture without expanding the secret's exposure surface.
pub fn log_oauth_capture(
    ...
    push_event(
        ...
        NetworkAuditEvent {
            ...
            mode: map_mode(mode),
            decision: NetworkAuditDecision::Allow,
            // OAuth capture fires on the response side and has no
            // managed-credential context; the rich audit fields are
            // intentionally None so the event is unambiguously a
            // capture record rather than a credential-injection one.
            route_id: None,
            auth_mechanism: None,
            auth_outcome: None,
...
mod tests {
    ...
    fn log_oauth_capture_records_event_with_reason() {
        ...
        assert_eq!(event.port, Some(443));
        assert_eq!(event.reason.as_deref(), Some("OAUTH_CAPTURE substituted=2"));
