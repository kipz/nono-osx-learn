...
mod tests {
    ...
    fn with_store_hydrates_existing_record_into_memory() {
        ...
        let broker = TokenBroker::with_store(store).expect("store load succeeds for valid record");
    ...
    fn with_store_propagates_load_errors() {
        ...
        impl crate::mediation::broker_store::BrokerStore for FailingStore {
            fn load(
                &self,
            ) -> nono::Result<Option<crate::mediation::broker_store::PersistedRecord>> {
            ...
            fn save(
                &self,
                _: &crate::mediation::broker_store::PersistedRecord,
            ) -> nono::Result<()> {
                Ok(())
            }
